...
const SelectWinner = () => {
    ...
    const contestData = useLoaderData();
        ...
        const winUrl = `/submission/contest-users-inner-join?email=${user.userEmail}`


        const { data: selectWinner = [], refetch } = useQuery({
            queryKey: ['selectwinner'],
            queryFn: async () => {
                const res = await axiosSecure.get(winUrl);
                return res.data;
            }
        })
        ...
        return (
            <>
                <DataTable
                    title="Users Participated Contests"
                    columns={columns}
                    data={selectWinner}
                    searchable={true}
